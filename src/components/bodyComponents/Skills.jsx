function Skills() {
	return (
		<div>
			<section id="skills">
				<h2>Skills</h2>
				<ul className="mylists">
					<li>Python, HTML, JavaScript, CSS</li>
					<li>Express, Django, Flask</li>
					<li>Node, React, Jest, npm, yarn</li>
					<li>Tailwind, Bootstrap, styled components</li>
					<li>Mongo, mongoose, SQL</li>
					<li>Pandas, Openpyxl, JSON, BSON, Excel</li>
					<li>BeautifulSoup, scrapy, selenium</li>
					<li>Rest API, Routers, HTTP, Axios, Fetch</li>
					<li>Bcrypt, CORS, morgan</li>
					<li>Git, GitHub, GitLens</li>
					<li>VsCode, PyCharm, Postman, HoppScotch</li>
				</ul>
			</section>
		</div>
	)
}

export default Skills
